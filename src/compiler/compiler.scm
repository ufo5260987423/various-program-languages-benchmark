  ...
  (let ((new-graph
         (set-map (lambda (x)
                    (let ((new-edges (closure (gnode-edges x))))
                      (unless (set-equal? new-edges (gnode-edges x))
                        (set! changed? #t))
          ...
          (cond ((or (char-alphabetic? c) (char-numeric? c))
                ...
                (else
                 (let ((n (character-encoding c)))
                   (loop (- i 1)
                         (cons #\_
                               (cons (hex->char (div n 16))
                                     (cons (hex->char (mod n 16)) l))))))))
        (lst->string l))))
        ...
        (if (pair? l)
            (loop (cdr l)
                  (let ((x (car l)))
                    (if (lbl? x)
                        n
                        (mod (+ (* n 10000) x) hash-table-length))))
            n)))
    (define (hash-bb bb)
      (let ((branch (bb-branch-instr bb)))
        ...
        (if (pair? l)
            ...
            (let ()
              ...
              (bbs-lbl-counter-set!
               bbs
               (make-counter
                (* (+ 1 (div (bbs-new-lbl! bbs) 1000)) 1000)
                label-limit
                bbs-limit-err))))))))
              ...
              (if (> n 0)
                  (begin
                    (string-set!
                     s
                     i
                     (string-ref "0123456789ABCDEF" (mod n 16)))
                    (loop (- i 1) (div n 16)))))
            (ofile-display s))))
    ...
    (let* ((fix-list
      ...
      (define (write-block start-loc end-loc start end)
        (if (> end-loc start-loc)
            (ofile-word (div (- end-loc start-loc) 2)))
...
(define (disp-offset x)
  (- (mod (+ (div (- x 40) 8) 32768) 65536) 32768))
...
(define (inx-offset x)
  (- (mod (+ (div (- x 524328) 128) 128) 256) 128))
...
(define (make-pcr lbl offset)
  (+ 557104 (+ (mod offset 65536) (* lbl 65536))))
    ...
    (let loop ((i 15) (bit 32768) (mask 0))
      (if (>= i 0)
          (loop (- i 1)
                (div bit 2)
                (if (memq i reg-list)
                    (+ mask (if flip-bits? (div 32768 bit) bit))
                    mask))
          mask)))
...
(define (dump-bignum x)
  ...
  (define (integer->digits n)
    (if (= n 0)
        '()
        (cons (mod n radix) (integer->digits (div n radix)))))
  ...
  (lambda (opnds loc sn)
    (let ((sn-loc (if loc (sn-opnd loc sn) sn)))
      (let* ((n (length opnds))
             ...
             (adjust (mod (- bytes) 8)))
        (gen-guarantee-space
         (div (* (div (+ bytes (- 8 1)) 8) 8) pointer-size))
